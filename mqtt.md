...
Both paths can run **at the same time**. Version 0.3.0 also adds a controller inside the add-on;
it publishes a native MQTT `lawn_mower` entity plus 1, 3 and 6-hour timed-start buttons and
forwards commands to the gateway's official local API.
...
Home Assistant then assigns the `lawn_mower` control, timed-start buttons and publisher sensors
to the same mower device automatically. No custom integration is needed. The Matter entity
remains read-only; commands use MQTT.
...
## Blocking GARDENA cloud access at the router

The reliable and reversible isolation point is the router, rather than stopping vendor services
on the gateway. In UniFi, create a block policy with the GARDENA gateway as the source device and
**External/Internet** as the destination; cover IPv4, IPv6 and all protocols. Keep the required
local Home Assistant and MQTT paths allowed separately.

| Source | Destination | Protocol/port |
|---|---|---|
| GARDENA gateway | MQTT broker | TCP 1883 |
| Home Assistant add-on | GARDENA gateway | TCP 22 and 443 |
| Home Assistant / Matter | GARDENA gateway | UDP 5540 |
| Browser / Home Assistant | GARDENA gateway | TCP 8099 or HTTPS 443 |

With VLANs, place these required local allowances before a broader inter-VLAN block. Then test the
GARDENA app over mobile data: after any cached values expire it should no longer reach the gateway,
while MQTT telemetry and local commands continue to work.
