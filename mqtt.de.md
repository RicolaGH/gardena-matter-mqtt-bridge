...
Beide Pfade können **gleichzeitig** laufen. Version 0.3.0 ergänzt außerdem einen Controller im
Add-on; er veröffentlicht eine native MQTT-`lawn_mower`-Entität sowie Zeitstart-Buttons für
1, 3 und 6 Stunden und leitet Befehle an die offizielle lokale Gateway-API weiter.
...
| SILENO Mähroboter | Status, Akku, Funkverbindungsqualität, Laufzeit, Fehlercode, Start/Parken, modellabhängig Pause sowie Zeitstart für 1/3/6 Stunden |
...
**Deploy / Re-Deploy** ausführen. Home Assistant ordnet `lawn_mower`-Steuerung,
Zeitstart-Buttons und Publisher-Sensoren automatisch demselben Mähergerät zu. Es ist keine
Custom-Integration nötig. Die Matter-Entität bleibt read-only; gesteuert wird über MQTT.
...
## GARDENA-Cloud am Router sperren

Die zuverlässige und reversible Trennung erfolgt am Router, nicht durch das Beenden von
Herstellerdiensten auf dem Gateway. In UniFi eine Block-Regel mit dem GARDENA-Gateway als
Quellgerät und **External/Internet** als Ziel anlegen; IPv4 und IPv6 sowie alle Protokolle
einschließen. Lokale Regeln zu Home Assistant und MQTT bleiben davon getrennt erlaubt.

Für die in diesem Projekt verwendeten lokalen Pfade müssen insbesondere erreichbar bleiben:

| Quelle | Ziel | Protokoll/Port |
|---|---|---|
| GARDENA-Gateway | MQTT-Broker | TCP 1883 |
| Home-Assistant-Add-on | GARDENA-Gateway | TCP 22 und 443 |
| Home Assistant / Matter | GARDENA-Gateway | UDP 5540 |
| Browser / Home Assistant | GARDENA-Gateway | TCP 8099 bzw. HTTPS 443 |

Bei VLANs die benötigten lokalen Freigaben vor einer allgemeineren Inter-VLAN-Sperre platzieren.
Danach die GARDENA-App über Mobilfunk testen: Nach Ablauf eventuell gecachter Werte darf sie das
Gateway nicht mehr live erreichen, während MQTT-Werte und lokale Befehle weiter funktionieren.
