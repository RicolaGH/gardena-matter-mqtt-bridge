...
The Matter bridge and MQTT sensor publisher run **standalone on the gateway**:
reboot-proof and OTA-surviving (via the restore service). Matter talks **directly
gateway ↔ Home Assistant**. When native mower control is enabled, the add-on remains
a small runtime component for commands because it maintains the encrypted local tunnel.

- If native mower control is disabled, you may **stop or uninstall** this add-on after
  a successful deploy — the **pairing stays intact** and gateway-hosted telemetry keeps
  working. Native Start/Dock/Pause commands require the add-on to stay running.
...
Version 0.3.0 publishes Start, Dock and, where the mower supports it, Pause as a
native Home Assistant MQTT `lawn_mower` entity. It also publishes **Start 1 h**,
**Start 3 h** and **Start 6 h** buttons. No separate custom integration is required.
...
For a single mower, the add-on adopts the existing MQTT publisher's retained device
identifier. Sensors, control and timed-start buttons therefore appear under one Home
Assistant device. On upgrading from 0.2.x, the old separate control discovery record
is removed automatically. The Matter pairing and `chip_kvs` are not changed.
...
Matter-Bridge und MQTT-Sensor-Publisher laufen **eigenstaendig auf dem Gateway**:
reboot-fest und OTA-ueberlebend (ueber den Restore-Dienst). Matter spricht **direkt
Gateway ↔ Home Assistant**. Bei aktivierter nativer Maeher-Steuerung bleibt das Add-on
als kleiner Laufzeit-Bestandteil fuer den verschluesselten lokalen Befehlstunnel aktiv.

- Wenn die native Maeher-Steuerung deaktiviert ist, darfst du das Add-on nach einem
  erfolgreichen Deploy **stoppen oder deinstallieren** — das **Pairing bleibt erhalten**
  und die Gateway-Telemetrie funktioniert weiter. Start/Parken/Pause benoetigen dagegen
  das laufende Add-on.
...
Version 0.3.0 stellt **Start**, **Parken** und, sofern das Maehermodell es
unterstuetzt, **Pause** als native Home-Assistant-MQTT-`lawn_mower`-Entitaet
bereit. Dazu kommen die Buttons **Start 1 h**, **Start 3 h** und **Start 6 h**.
Es wird keine separate Custom-Integration installiert oder benoetigt.
...
Bei genau einem Maeher uebernimmt das Add-on die vorhandene, retained publizierte
MQTT-Geraetekennung. Sensoren, Steuerung und Zeitstart-Buttons erscheinen gemeinsam
in einem Home-Assistant-Geraet. Beim Update von 0.2.x wird der alte separate
Steuerungs-Eintrag automatisch entfernt. Matter-Pairing und `chip_kvs` bleiben unveraendert.
